...
with tab3:
    ...
    # Inisialisasi state
    for key, default in [("has_calculated", False), ("x_input", ""), ("y_input", ""), ("y_sampel", 0.0)]:
    ...
    if not st.session_state.has_calculated:
        ...
        # Proses regresi
        if st.session_state.x_input and st.session_state.y_input:
            try:
                x_vals = np.array([float(v) for v in st.session_state.x_input.split(",")])
                y_vals = np.array([float(v) for v in st.session_state.y_input.split(",")])
                if len(x_vals) != len(y_vals):
                    ...
                    # Clear Regresi button
                    if st.button("❌ Clear Regresi"):
                        for k in ["has_calculated","x_input","y_input","y_sampel"]:
                            st.session_state[k] = False if isinstance(st.session_state[k], bool) else "" if isinstance(st.session_state[k], str) else 0.0
                        st.experimental_rerun()
